	...
	<div class="layout">
		...
		<!-- 로그인 했을 때 헤더 -------------------------------------------------------------------------------------------------------------------------------------------------------- -->					
							<sec:authorize access="isAuthenticated()">
								...
								<a class="layout-navigation-bar-icon layout-navigation-bar-icon--hide-mobile" title="스크랩북" aria-label="스크랩북" href="./myScrap?memberNum=${mem.memberNum}&id=${mem.id}">
	...
	<!-- 바디 -->	
		...
		<div class="css-1hs06u0-LayoutContainer e1ivswjr2 closeMenu">
			...
			<div class="css-1hrluq4-Container ekbdbra0" >
				...
				<c:forEach items="${myPic}" var="myPic" varStatus="status">
										...
										<article class="card-collection-item">
											...
											<div id="card-collection-item-11103723" class="card-collection-item__content">
												...
												<div class="card-collection-item__image">
													...
													<c:forEach items="${myPic.pictureFileVO}" var="file" varStatus="fileIndex">
													<c:if test="${fileIndex.index eq 0}">
														<img class="image" alt="" src="../resources/upload/picture/${file.picFilename}">
													</c:if>
													</c:forEach>
														<span class="card-item-image__view-count">조회수 ${myPic.hit}</span>
													</div>
												</div>
												</div>
