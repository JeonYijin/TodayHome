...
<link rel="stylesheet" href="../resources/css/mySetting.css">
<link rel="stylesheet" href="../resources/css/myNoti.css">
...
<!--  바디 -->
		<div>
		<input type="hidden" value="${member.memberNum}" id="memNum">
		<input type="hidden" value="${noti.noti_email}" id="noti_e_value">
		<input type="hidden" value="${notiM.noti_msg}" id="noti_m_value">
			<div class="css-duqdc3-PushSettingContainer e1ok56tx5">
				<h1 class="css-p80mfr-PushSettingTitle e1ok56tx4">이벤트 및 혜택 알림</h1>
				<p class="css-114agji-PushSettingSubTitle e1ok56tx3">특가, 쿠폰 등 이벤트 정보를 빠르게 알려드릴게요.</p>
				<ul class="css-1xmpys0-PushSettingList e1ok56tx2">
					<li class="css-n4hdvq-PushSettingItem e1ok56tx1">
						<span class="css-glpnvo-PushSettingItemName e1ok56tx0">이메일</span>
						<c:if test="${noti.noti_email eq null || noti.noti_email eq 0 }">
							<span class="css-1xnppoi-SwitchSpan e1pegdba2 emailSpan">
								<input type="checkbox" name="isEmailSubscribed" class="css-xd21da-SwitchInput e1pegdba0 emailInput" value="">
								<span class="css-10jjmn2-SwitchSpanThumb e1pegdba1"></span>
							</span>
						</c:if>
						<c:if test="${noti.noti_email eq 1 }">
							<span class="css-1xnppoi-SwitchSpan e1pegdba2 emailSpan">
								<input type="checkbox" name="isEmailSubscribed" class="css-xd21da-SwitchInput e1pegdba0 emailInput" checked="checked">
							<span class="css-10jjmn2-SwitchSpanThumb e1pegdba1"></span>
						</span>
						</c:if>
					...
					<li class="css-n4hdvq-PushSettingItem e1ok56tx1">
						<span class="css-glpnvo-PushSettingItemName e1ok56tx0">문자 메시지</span>
						<c:if test="${notiM.noti_msg eq null || notiM.noti_msg eq 0 }">
							<span class="css-1xnppoi-SwitchSpan e1pegdba2 msgSpan">
								<input type="checkbox" name="isSmsSubscribed" class="css-xd21da-SwitchInput e1pegdba0 msgInput" value="">
								<span class="css-10jjmn2-SwitchSpanThumb e1pegdba1"></span>
							</span>
						</c:if>
						<c:if test="${notiM.noti_msg eq 1 }">
							<span class="css-1xnppoi-SwitchSpan e1pegdba2 msgSpan">
								<input type="checkbox" name="isSmsSubscribed" class="css-xd21da-SwitchInput e1pegdba0 msgInput" checked="checked">
								<span class="css-10jjmn2-SwitchSpanThumb e1pegdba1"></span>
							</span>
						</c:if>
...
// -----------------------------------------------------------------------------------
let noti_e_value = $('#noti_e_value').val();
let noti_m_value = $('#noti_m_value').val();





if(noti_e_value == null || noti_e_value == 0){
	//이메일 수신 - 안되어있을 때
	$('.emailSpan').click(function(){
		//alert('이메일')
		$('.emailInput').attr('value', 1);
		let noti_email= $('.emailInput').val(); 
		let memberNum = $('#memNum').val();
		console.log(noti_email, memberNum);
		...
		//디비에 email값 저장
		 $.ajax({
			...
			success:function(result){
				console.log('이메일 수신 동의')
				alert('이메일 수신에 동의하셨습니다.')
...
if(noti_m_value == null || noti_m_value == 0){
	//문자메시지 - 안되어있을때
	$('.msgSpan').click(function(){
		//alert('메시지');
		$('.msgInput').attr('value', 1);
		let noti_msg = $('.msgInput').val();
		let memberNum = $('#memNum').val();
		...
		//디비에 email값 저장
		 $.ajax({
			...
			success:function(result){
				console.log('문자메시지 수신 동의')
				alert('문자메시지 수신에 동의하셨습니다.')
				document.location.reload(true);
			},
			error:function(error){
				console.log(error)
